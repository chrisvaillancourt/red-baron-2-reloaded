...
| 4 | minor | **An escort is judged a success when the player dies early.** The N.65 pilot collided at 4:47, before the Farmans reached their objective, and the report said MISSION SUCCESSFUL. The check seems to count charges still alive when the flight ends. | Career escort; die early | **Fixed** (lead): at the end, an escort counts only if its charges have been out over the lines (`missionDirector.test.ts`). |
