...
interface Acc {
  ...
  shots: number; hits: number; hitsHeadOn: number; hitsTail: number; speed: number; speedN: number; energyLoss: number; stall: number; recover: number; defend: number;
}
const zero = (): Acc => ({ engage: 0, near: 0, nose30: 0, nose10: 0, solution: 0, firing: 0, shots: 0, hits: 0, hitsHeadOn: 0, hitsTail: 0, speed: 0, speedN: 0, energyLoss: 0, stall: 0, recover: 0, defend: 0 });
        ...
        while (!core.director.ended && core.world.time < 900) {
          ...
          if (TRACE && r === 0 && tick % 240 === 0 && core.world.time < TRACE) {
            const row = core.world.aircraft.filter((a) => !a.outcome).map((a) => {
              ...
              const t = ctl?.targetId != null ? byId.get(ctl.targetId) : undefined;
              let rel = '';
              if (t) {
                const d = t.state.position.clone().sub(a.state.position);
                const fw = forwardOf(a.state.orientation, new Vector3());
                rel = ` ->#${t.id} ${d.length().toFixed(0)}m off${(Math.acos(Math.min(1, fw.dot(d.normalize()))) * 57.3).toFixed(0)} dh${(a.state.position.y - t.state.position.y).toFixed(0)}`;
              }
              return `${a.id === pl.id ? 'P' : a.side[0]}${a.id}:${a.spec.shortName} ${ctl?.debugState ?? '-'} h${a.state.position.y.toFixed(0)} v${a.state.airspeed.toFixed(0)}${rel}`;
            });
            process.stdout.write(`t=${core.world.time.toFixed(0)} ${row.join(' | ')}\n`);
          }
