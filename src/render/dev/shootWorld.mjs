...
const SHOTS = [
  { name: 'arras-800', q: (process.env.EXTRA ?? '') + 'town=Arras&alt=800&pitch=-14&date=1917-06-01&tod=morning&cover=0.35' },
...
for (const s of SHOTS) {
  ...
  const t0 = Date.now();
  await page.goto(`http://localhost:${port}/dev/world.html?${s.q}`);
